...
namespace Neat.BibTeX.BibModel
{
  ...
  /// An instance could be <see cref="Bib8StringEntry"/>,
  /// <see cref="Bib8PreambleEntry"/>, or
  /// <see cref="Bib8GeneralEntry"/>
  /// (e.g., <c>@type{ ... }</c>).
  /// </summary>
  public abstract class Bib8Entry
  {
    /// <summary>
    ...
    private protected Bib8Entry(StringT type, bool isBrace)
    {
      Type = type;
      IsBrace = isBrace;
    }
    ...
    /// </summary>
    public abstract void AcceptVisitor<TVisitor>(ref TVisitor visitor) where TVisitor : struct, IBib8EntryVisitor;

    /// <summary>
    /// Dispatches the correct method for the reference-type visitor.
    /// </summary>
    public abstract void AcceptVisitor(IBib8EntryVisitor visitor);
