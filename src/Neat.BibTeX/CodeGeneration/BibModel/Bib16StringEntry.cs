...
namespace Neat.BibTeX.BibModel
{
  ...
  public sealed class Bib16StringEntry : Bib16Entry
  {
    ...
    internal void CtorCheckImpl(string name)
    {
      ...
      {
        throw new ArgumentException("Bib16StringEntry: Type is not \"string\".", name is null ? "type" : name);
      }
      if (!BibBstChars.IsIdentifier(Name))
      {
        throw new ArgumentException("Bib16StringEntry: Name is not a valid identifier.", name is null ? "name" : name);
      }
