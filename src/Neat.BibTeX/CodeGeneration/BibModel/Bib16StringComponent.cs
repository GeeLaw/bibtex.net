  ...
  public readonly struct Bib16StringComponent
  {
    ...
    internal void CtorCheckImpl(string name)
    {
      ...
      case BibStringComponentType.NameValue:
        ...
        {
          throw new ArgumentException("Bib16StringComponent: NameOrLiteral is not a valid identifier.", name is null ? "nameOrLiteral" : name);
        }
        break;
      case BibStringComponentType.QuoteLiteralValue:
        if (!BibBstChars.IsQuoteLiteral(nameOrLiteral))
        {
          throw new ArgumentException("Bib16StringComponent: NameOrLiteral is not a valid quote-delimited literal.", name is null ? "nameOrLiteral" : name);
        }
        break;
      case BibStringComponentType.NumericLiteralValue:
        if (!BibBstChars.IsNumericLiteral(nameOrLiteral))
        {
          throw new ArgumentException("Bib16StringComponent: NameOrLiteral is not a valid numeric literal.", name is null ? "nameOrLiteral" : name);
        }
        break;
      case BibStringComponentType.BraceLiteralValue:
        if (!BibBstChars.IsBraceBalanced(nameOrLiteral))
        {
          throw new ArgumentException("Bib16StringComponent: NameOrLiteral is not brace-balanced.", name is null ? "nameOrLiteral" : name);
        }
        break;
      default:
        throw new ArgumentException("Bib16StringComponent: Type is not valid.", name is null ? "type" : name);
      }
#endif
    }
