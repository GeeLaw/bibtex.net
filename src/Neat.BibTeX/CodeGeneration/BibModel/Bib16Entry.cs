...
namespace Neat.BibTeX.BibModel
{
  ...
  /// An instance could be <see cref="Bib16StringEntry"/>,
  /// <see cref="Bib16PreambleEntry"/>, or
  /// <see cref="Bib16GeneralEntry"/>
  /// (e.g., <c>@type{ ... }</c>).
  /// </summary>
  public abstract class Bib16Entry
  {
    /// <summary>
    ...
    private protected Bib16Entry(StringT type, bool isBrace)
    {
      Type = type;
      IsBrace = isBrace;
    }
    ...
    /// </summary>
    public abstract void AcceptVisitor<TVisitor>(ref TVisitor visitor) where TVisitor : struct, IBib16EntryVisitor;

    /// <summary>
    /// Dispatches the correct method for the reference-type visitor.
    /// </summary>
    public abstract void AcceptVisitor(IBib16EntryVisitor visitor);
